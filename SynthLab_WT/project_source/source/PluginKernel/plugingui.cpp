...
CView* PluginGUI::createUserCustomView(std::string viewname, const CRect rect, IControlListener* listener, int32_t tag)
{
	...
	{
		if (viewname.find("_LA") != std::string::npos)
			return new CustomTextLabel(rect, CHoriTxtAlign::kLeftText);

		if (viewname.find("_CA") != std::string::npos)
			return new CustomTextLabel(rect, CHoriTxtAlign::kCenterText);

		if (viewname.find("_RA") != std::string::npos)
			return new CustomTextLabel(rect, CHoriTxtAlign::kRightText);
...
CView* PluginGUI::createView(const UIAttributes& attributes, const IUIDescription* description)
{
	...
	// --- timing note duration control
	if (viewname == "ReadOnlyKnob")
	{
		// --- our wave view testing object
		const std::string* sizeString = attributes.getAttributeValue("size");
		const std::string* originString = attributes.getAttributeValue("origin");
		...
		CReadOnlyKnob* customKnob = new CReadOnlyKnob(rect, listener, tag, subPixmaps, heightOfOneImage, pBMP, offsetPoint);
