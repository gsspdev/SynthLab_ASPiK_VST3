...
tresult PLUGIN_API VST3Plugin::initialize(FUnknown* context)
{
	...
	if(result == kResultTrue)
	{
        ...
        for (uint32_t i = 0; i < ControllerNumbers::kCountCtrlNumber /*midiCCMapping.size()*/; i++)
        {
            ...
            param = new RangeParameter(USTRING(paramName.c_str()), baseCCParamID + i, USTRING(""), 0.0, 1.0, defaultValue);
            parameters.addParameter(param);
        }
...
bool VST3Plugin::issueMIDICCProxyMessage(ParamID proxyParamID, ParamValue proxyParamValue)
{
 ...
 /*       uint32_t midiDataLSB = 0;
        uint32_t midiDataMSB = 0;*/
        //unipolarDoubleToMIDI14_bit(proxyParamValue, midiDataLSB, midiDataMSB);
        unipolarDoubleToMIDI14_bit(proxyParamValue, event.midiData1, event.midiData2);
  /*      event.midiData1 = midiDataLSB;
        event.midiData2 = midiDataMSB;*/
        midiEventQueue->addMIDIProxyEvent(event);
        return true;
    }
